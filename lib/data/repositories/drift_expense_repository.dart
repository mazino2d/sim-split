...
import '../../domain/failures/expense_failure.dart';
...
class DriftExpenseRepository implements ExpenseRepository {
    ...
    return _expenseDao
        ...
        .asyncMap((rows) async {
          ...
          for (final row in rows) {
            final splits =
                await _expenseSplitDao.getSplitsForExpense(row.id);
  ...
  Future<Either<Failure, Expense>> addExpense(Expense expense) async {
    try {
      ...
      await _expenseSplitDao.insertSplits(
          _mapper.splitCompanions(expense.splits));
